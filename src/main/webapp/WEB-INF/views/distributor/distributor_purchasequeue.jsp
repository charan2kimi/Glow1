...
$("#wrdetails").append('<tr><th>Order Item ID</th><th>Name</th><th>Compound</th><th>Quantity</th><th>Manufacturer</th></tr>');
for(var i in data) {
	
	
	  if(!isNaN(i)) {
			$("#wrdetails").append('<tr><td>'+data[i].id+'</td><td >'+data[i].name+'</td><td>'+data[i].compound+'</td><td>'+data[i].quantity+'</td><td>'+data[i].manufacturer+'</td></tr>');
	  }}
});	
});
...
$('#wrtable').on('click', '.processWR', function () {
	...
	$.getJSON("processWR", {wid:wrid }, function (data){
