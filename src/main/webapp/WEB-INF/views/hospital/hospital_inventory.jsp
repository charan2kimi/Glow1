...
$('#packageTable').on('click', '.reportPackage', function () {
	...
	$.getJSON("reportPackage", {id:pid }, function (data)
		...
		for(var i in data) {
			  ...
			  if(!isNaN(i)) {
				  $("#packageTable").append('<tr><td>'+data[i].id+'</td><td >'+data[i].drug.name+'</td><td>'+data[i].drug.manufacturer+'</td><td>'+data[i].status+'</td><td><input type="button" class="reportPackage" id="'+data[i].id+'" value="Report"></td></tr>');
			  }}
	});	
});
